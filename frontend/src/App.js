...
const Shell = () => {
  ...
  const [appVersion, setAppVersion] = useState({ current: "1.0.5", latest: "1.0.5", update_available: false });
  ...
  const load = useCallback(async () => {
    const [adminRes, setupRes, serverList, schemaRes, versionRes] = await Promise.all([
      ...
      endpoints.getAppVersion().catch(() => ({ current: "1.0.5", latest: "1.0.5", update_available: false })),
    ]);
