...
## Recent Changes
- **2026-02 (build.ps1 hardening)**: Backend build script now auto-installs `requirements.txt` before PyInstaller and sanity-checks critical imports (`discord`, `fastapi`, `uvicorn`, `motor`, `aiohttp`). Fixes root cause of silent Windows Server 2019 / Win10 startup crash: stale venv lacked `discord.py`, `collect_submodules('discord')` silently returned `[]`, exe shipped without the module, Electron splash hung on "Backend hazır olmayı bekliyor…". Always cleans `build/` + `dist/` to prevent stale bytecode. User fix: re-run `npm run dist` inside `/electron`.
- **2026-02 (Electron error surface)**: Reverted `dialog.showMessageBox` → `dialog.showErrorBox` in `electron/main.js` so init failures show a visible modal instead of silently killing the splash (root cause of earlier "no window, no log" reports on Server 2019).
